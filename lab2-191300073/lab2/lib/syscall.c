...
char getChar(){ // 对应SYS_READ STD_IN
	...
	char ret;
	ret=syscall(SYS_READ, STD_IN,0,0, 0, 0);
	return ret;
}
...
void printf(const char *format,...){
	...
	while(format[i]!=0){
		...
		if(format[i]=='%'){
			...
			count--;//指到%字符
			i++;
			if(format[i]=='%'){//%%d输出%d %%d ,a warning %%输出%
				state=0;
				buffer[count++]=format[i];
			}
			else{
				...
				switch(format[i]){
					...
					// 	decimal=decimal/10;
					// }while (decimal!=0);
					// for(int i=k-1;i>=0;i--)	buffer[count++]=num[i];
					count=dec2Str(decimal,buffer,256,count);
					break;
					}
					case'x':{
						...
						// buffer[count++]='0';
						// buffer[count++]='x';
						count=hex2Str(hexadecimal,buffer,256,count);
						break;
					}
					case's':
						string = *(char**)(paraList + index);
						count=str2Str(string,buffer,256,count);
						break;
					case'c':
						character = *(char*)(paraList + index);
						buffer[count++]=character;
						if(count==256){
							syscall(SYS_WRITE, STD_OUT, (uint32_t)buffer,256, 0, 0);
							count=0;
						}
						break;
					default:	state=2;
