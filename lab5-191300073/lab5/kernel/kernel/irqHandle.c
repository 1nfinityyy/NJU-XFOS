...
void syscallOpen(struct StackFrame *sf) {
	...
	if (ret == 0) { // file exist
		...
		for(i = 0; i < MAX_DEV_NUM; i++) {
			if(dev[i].inodeOffset == destInodeOffset && dev[i].state == 1)//设备在使用
			{
				dev[i].value--;
				if(dev[i].value < 0){
...
void syscallWrite(struct StackFrame *sf) {
	...
	if(sf->ecx>=4 && sf->ecx<8 && file[sf->ecx-4].state==1){
		syscallWriteFile(sf);
	}
	return;
}
...
void syscallWriteFile(struct StackFrame *sf) {
	...
	while(left_num)
	{
		...
		{
			ret = allocBlock(&sBlock,gDesc,&inode, file[index].inodeOffset);
			if(ret<0)
			{
				inode.size = offset + size - left_num > inode.size ? offset + size - left_num : inode.size;
				sf->eax = size - left_num;
				diskWrite(&inode, sizeof(Inode), 1, file[index].inodeOffset);
				return;	
			}
		}
...
void syscallReadFile(struct StackFrame *sf) {
	...
	uint8_t buffer[SECTOR_SIZE * SECTORS_PER_BLOCK];
	int quotient = file[sf->ecx - MAX_DEV_NUM].offset / sBlock.blockSize;//块号
	int remainder = file[sf->ecx - MAX_DEV_NUM].offset % sBlock.blockSize;//偏移
	...
	j=remainder;
	i=0;
	int block_index=quotient;
	int left_size=size;
	int start=j;
	while(left_size>0){
		int free_size = sBlock.blockSize - j;
		if (free_size + file[sf->ecx - MAX_DEV_NUM].offset >= inode.size){
			for (int k = 0; k < inode.size - file[sf->ecx - MAX_DEV_NUM].offset; k++)
			{
				str[i++] = buffer[start++];
			}
			pcb[current].regs.eax = size - left_size + inode.size - file[sf->ecx - MAX_DEV_NUM].offset;
			file[sf->ecx - MAX_DEV_NUM].offset=inode.size;
			return;
		}
		if (free_size >= left_size)
		{
			for (int k = 0; k < left_size; k++)
			{
				str[i++] = buffer[start++];
			}
			file[sf->ecx - MAX_DEV_NUM].offset += left_size;
			pcb[current].regs.eax = size;
			return;
		}
		else
		{
			...
			block_index++;
			file[sf->ecx - MAX_DEV_NUM].offset += free_size;
			left_size -= free_size;	
			j = 0;
			start = j;
...
void syscallLseek(struct StackFrame *sf) {
	...
	switch(sf->ebx) { // whence
		...
		case SEEK_END:
			// TODO: if SEEK_END
			file[sf->ecx - MAX_DEV_NUM].offset = inode.size + offset;
			break;
		default:
			pcb[current].regs.eax = -1;
			return;
...
void syscallRemove(struct StackFrame *sf) {
	...
	if (ret == 0) { // file exist
		...
		else	stringCpy(str, fatherPath, size);	
		char name[stringLen(str + size + 1)+1];
		for(int k=0;k<stringLen(str + size + 1);k++){
			name[k]=*(str+size+k+1);
		}
		name[stringLen(str + size + 1)]=0;
		ret = readInode(&sBlock,gDesc, &fatherInode, &fatherInodeOffset, fatherPath);
		if (destInode.type == REGULAR_TYPE) {
			// TODO: If REGULAR_TYPE
			ret = freeInode(&sBlock, gDesc, &fatherInode, fatherInodeOffset, &destInode, &destInodeOffset, name, REGULAR_TYPE);
		}
		else if (destInode.type == DIRECTORY_TYPE) {
			// TODO: If DIRECTORY_TYPE
			ret = freeInode(&sBlock, gDesc, &fatherInode, fatherInodeOffset, &destInode, &destInodeOffset, name,DIRECTORY_TYPE);
		}
		if (ret == -1) {
			pcb[current].regs.eax = -1;
			return;
